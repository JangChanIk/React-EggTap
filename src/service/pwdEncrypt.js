...
{ 
  ...
  const key512Bits10000Iterations = CryptoJS.PBKDF2('0000', CryptoJS.enc.Hex.parse(salt), { keySize: 512 / 32, iterations, }); 
  const encrypted = CryptoJS.AES.encrypt( pwd, key512Bits10000Iterations, { iv: CryptoJS.enc.Hex.parse(iv) }, ); 
  return { encryptedPwd: encrypted.toString(), salt, iv, }; 
};

export default pwdEncrypt;

/* export const checkIdPwValid = () => 
{ 
